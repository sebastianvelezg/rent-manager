...
import { renterNotifications } from "@/lib/renter-mock-data";
import { RenterNotification } from "@/types/renter";
import Link from "next/link";

export default function RenterNotificationsPage() {
  const [notifications, setNotifications] =
    useState<RenterNotification[]>(renterNotifications);
  const [filter, setFilter] = useState<"all" | "unread">("all");

  const unreadCount = notifications.filter((n) => !n.read).length;

  const getNotificationIcon = (type: RenterNotification["type"]) => {
    switch (type) {
      case "payment":
  ...
  const getNotificationColor = (type: RenterNotification["type"]) => {
    switch (type) {
      case "payment":
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        <p className="text-xs text-muted-foreground">
                          {format(notification.timestamp, "d MMM yyyy, HH:mm", {
                            locale: es,
                          })}
                        </p>
